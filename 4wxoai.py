...
class MyFrame(wx.Frame):
	def __init__(self):
		...
		b_hbox = wx.BoxSizer(wx.HORIZONTAL)
		if 1:
			...
			b_hbox.Add(self.add_question_btn, flag=wx.ALL | wx.CENTER, border=10)

		if 1:
			self.ask_btn = wx.Button(self.content_panel, label="Ask GPT-4")
			AsyncBind(wx.EVT_BUTTON, self.on_chat, self.ask_btn)
			b_hbox.Add(self.ask_btn, flag=wx.ALL|wx.CENTER, border=10)
		if 1:
			self.timer_output = wx.StaticText(panel, label="")
			b_hbox.Add(self.timer_output, flag=wx.ALL|wx.CENTER, border=10)
		if 1:
			self.button = wx.Button(self.content_panel, label="Highlight", pos=(10,40))
			self.button.Bind(wx.EVT_BUTTON, self.on_button_click)
			b_hbox.Add(self.button, flag=wx.ALL|wx.CENTER, border=10)
		vbox.Add(b_hbox, flag=wx.ALL|wx.CENTER, border=10)
	...
	def on_button_click(self, event):
		...
		self.input_text.SetSelection(-1, -1)	
		if 0:
			start_pos = content.find('world')
			
			if start_pos != -1:
				end_pos = start_pos + len('world')
				self.textctrl.SetFocus()  # Set focus to the TextCtrl first
	...
	def OnClick(self, event):
		...
		start, end = idata.get('selection',[0,0])
		pp(idata)
		if start!=end:
		
			def do_highlight(sel):
				
				self.input_text.SetFocus()
				self.input_text.SetSelection(*sel)
			
			wx.CallLater(100, do_highlight, (start, end))
		#self.input_text.SetFocus()
	...
	def load_questions_into_tree(self):
		...
		for filename in sorted([join(self.questions_dir,f) for f in files], key=os.path.getctime):
			...
			for lid,log_question in enumerate(log_questions):
				...
				self.tree.SetItemData(log_question_item, dict(log_id=lid,filename=filename,level='log', **log_question))
	...
	def populate_log(self, filename):
		...
		with open( filename, 'r', encoding='utf-8') as file:
			...
			for log_question in log_questions:
				...
				self.log_item_dict[new_item] = dict(filename=filename,level='log', **log_question)
	...
	async def on_chat(self, event):
		...
		selection=start, end = self.input_text.GetSelection()
		full_question = self.input_text.GetValue()
		...
		if os.path.exists(selected_file):
			...
			data['question'] = full_question
			data['answer'] = response
			data['selection'] = selection
			
			with open(selected_file, 'w', encoding='utf-8') as f:
				json.dump(data, f, ensure_ascii=False, indent=4)
			self.add_answer_question_pair(selected_file,full_question, response, selection)
		wx.EndBusyCursor()
		self.timer.Stop()
		self.on_api_call_complete()

	#@timer_decorator
	async def __chat(self, content):		
		...
		async with aiohttp.ClientSession() as session:
			async with session.post(url, headers=headers, json=data) as response:
				if response.status == 200:
					...
					# Append system's message to the history
					messages.append({"role": "system", "content": resp_json["choices"][0]["message"]["content"]})

					return resp_json["choices"][0]["message"]["content"]
				else:
					error_msg = f"Error: {await response.text()}"
					# Optionally, append the error to the message history to keep track
					# self.message_history.append({"role": "system", "content": error_msg})
					return error_msg

			
	@timer_decorator
	async def _chat(self, content):
		url = "https://api.openai.com/v1/chat/completions"
		headers = {
			"Authorization": f"Bearer {openai.api_key}",
			"Content-Type": "application/json"
